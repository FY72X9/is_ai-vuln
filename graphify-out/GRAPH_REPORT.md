...
- `run_tests()` --calls--> `compute_nemenyi_critical_difference()`  [INFERRED]
  D:\Codes\research_banks\is_ai-vuln\scratch\test_full_pipeline.py → D:\Codes\research_banks\is_ai-vuln\src\evaluation\statistical_tests.py
- `run_tests()` --calls--> `run_closed_loop_fuzzy_dematel()`  [INFERRED]
  D:\Codes\research_banks\is_ai-vuln\scratch\test_full_pipeline.py → D:\Codes\research_banks\is_ai-vuln\src\dematel\fuzzy_engine.py
- `run_tests()` --calls--> `run_monte_carlo_sensitivity_proof()`  [INFERRED]
  D:\Codes\research_banks\is_ai-vuln\scratch\test_full_pipeline.py → D:\Codes\research_banks\is_ai-vuln\src\dematel\causal_validation.py
...
Cohesion: 0.06
Nodes (25): BaseIDSModel, Abstract Base Class for all benchmarked IDS models., BaseIDSModel, LightGBMIDS, Classical Gradient Boosted Decision Tree (GBDT) Baselines. Implements XGBoost an, LightGBM Classifier Baseline with Histogram Gradient Optimization., XGBoost Classifier Baseline with Optuna Tuning and Hist/GPU acceleration., XGBoostIDS (+17 more)

### Community 1 - "Community 1"

Cohesion: 0.06
Nodes (32): Release weights and garbage collect GPU/CPU memory., Run DirectLiNGAM causal discovery and calculate SHD against Fuzzy DEMATEL digrap, run_causal_triangulation(), get_model(), export_benchmark_to_latex(), Automated LaTeX Table Generation for Q1 Journal Publication. Generates professio, Format DataFrame into publication-grade LaTeX table with booktabs., calculate_ttf_utility() (+24 more)

### Community 2 - "Community 2"

Cohesion: 0.06
Nodes (35): download_file(), ensure_gitignore_safeguards(), generate_synthetic_benchmark_sample(), initialize_dataset_directories(), is_synthetic_path(), prepare_benchmark_dataset(), Drive Dataset Retrieval, Checksum Verification & Automatic .gitignore Safeguards, Check and automatically append required exclusion rules to .gitignore. (+27 more)

### Community 3 - "Community 3"

Cohesion: 0.07
Nodes (33): construct_axiomatic_prior_matrix(), Axiomatic Theoretical Prior Matrix (W_theory) for Autonomous Closed-Loop Fuzzy D, Derive the 8x8 theoretical causal prior matrix W_theory.          Self-influence, _calculate_kendalls_w(), compute_structural_hamming_distance(), Monte Carlo Robustness Proof & DirectLiNGAM Algorithmic Causal Triangulation. Va, Execute 10,000 Monte Carlo perturbation iterations to prove stability of promine, Calculate Kendall's W concordance coefficient across m raters for n items. (+25 more)

### Community 4 - "Community 4"

Cohesion: 0.11
Nodes (30): main(), Script to apply standardized Google Drive + Local figure destination folders and, update_cell_2(), update_nb01(), update_nb02(), update_nb03(), update_nb04(), update_nb05() (+22 more)
...
Cohesion: 0.33
Nodes (12): find_code_cell(), indent_block(), load_nb(), main(), Post-generation upgrader: converts Notebook 01 / 02 / 04 from single-dataset ex, Return index of first code cell whose joined source contains marker., save_nb(), set_cell_source() (+4 more)
...
Cohesion: 0.67
Nodes (1): Semantic spot-check that the multi-dataset upgrades landed in the notebook artif

### Community 17 - "Community 17"

Cohesion: 0.67
Nodes (1): Functional test for save_publication_figure_dual across local and mock Google Dr
